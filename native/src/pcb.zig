...
pub fn start_pcb(writer: *std.Io.Writer) !void {
...
pub fn end_pcb(writer: *std.Io.Writer) !void {
    try writer.writeAll(")\n");
}

pub fn start_xx_poly(kind: []const u8, writer: *std.Io.Writer) !void {
    try writer.print("  ({s}_poly\n", .{kind});
    try writer.writeAll("    (pts\n");
}

pub fn add_xx_poly_point(pt: geometry.Point, scale_factor: f64, writer: *std.Io.Writer) !void {
    try writer.print("      (xy {d:.3} {d:.3})\n", .{ pt.x * scale_factor, pt.y * scale_factor });
}

pub fn end_xx_poly(layer: []const u8, width: f64, fill: bool, writer: *std.Io.Writer) !void {
    try writer.writeAll("    )\n");
    try writer.print("    (layer \"{s}\")\n", .{layer});
    try writer.print("    (width {d:.3})\n", .{width});
    try writer.print("    (fill {s})\n", .{if (fill) "solid" else "none"});
    try writer.print("    (tstamp \"{f}\")\n", .{FauxUUID.init()});
    try writer.writeAll("  )\n");
}

pub fn points_to_xx_poly(kind: []const u8, pts: []geometry.Point, scale_factor: f64, layer: []const u8, width: f64, fill: bool, writer: *std.Io.Writer) !void {
...
pub fn polylist_to_footprint(polylist: PolyList, layer: []const u8, scale_factor: f64, writer: *std.Io.Writer) !void {
...
pub fn add_drill(x: f64, y: f64, d: f64, scale_factor: f64, writer: *std.Io.Writer) !void {
    ...
    try writer.print("(clearance 0.1) (zone_connect 0) (tstamp {f}))", .{FauxUUID.init()});
